...
class Rectangle {
    constructor(centerPosXY, dimensions, color) {
        ...
        this.dimensions = {...this.originalDimensions }; //relative dimension of the rectangle in the canvas
        this.ratio = this.originalDimensions.height / this.originalDimensions.width; // when the width is modified, the height stay proportional
...
class Circle {
    constructor(nbRectangle, elementDimensions, radius, rotationSpeed, centerPosXY, reversePerspective, turnLeft, perspectiveFactor) {
    ...
    updateSize() {
        for (let i = 0; i < this.nbRectangle; i++) {
            this.rectangles[i].dimensions.width = this.rectangles[i].originalDimensions.width + Math.sin(this.rectangles[i].angleAroundCircle) * this.perspectiveFactor; //30 is arbitrary. Cause bug if this.elementDimensions.width is inferior
...
circles.push(new Circle(5, { width: 50, height: 100 }, 300, 0.02, { x: center.x - 300, y: center.y - 200 }, false, false, 30)); //circle param : nbRectangles , dimensions of rectangles, radius of circle, rotation speed, position of circle, reverse persepctive, turn left instead of right
circles.push(new Circle(5, { width: 50, height: 100 }, 300, 0.02, { x: center.x + 300, y: center.y + 200 }, true, true, 30));
